...
- **Real-time color grading** (Lightroom-style, applied after filters to preview / photo / video alike): exposure, contrast, highlights/shadows, whites/blacks, temperature/tint, vibrance/saturation, plus 6-band HSL hue/saturation/luminance (red/yellow/green/cyan/blue/magenta) and 3 one-tap presets. Zero-cost when untouched (identity skips the GL pass); persisted across restarts.
- **Tone curves** (PS-style): draggable monotonic cubic-spline curves for master RGB and R/G/B channels — press empty space to add points, drag to shape, long-press to delete. Baked into a 256-entry LUT texture applied first inside the grading pass, so photos/videos get exactly what the preview shows.
